...
Thanks, {{store_name}}'
  ...
  private function store_locale_analytics($pixel_id="",$google_id="")
  {
    return array("pixel_id"=>$pixel_id,"google_id"=>$google_id);
  }
  ...
  public function store($store_unique_id=0)
  {
    ...
    $data["social_analytics_codes"] = $this->store_locale_analytics($store_data[0]["pixel_id"],$store_data[0]["google_id"]);
  ...
  public function product($product_id=0)
  {
    ...
    $data["social_analytics_codes"] = $this->store_locale_analytics($product_data[0]["pixel_id"],$product_data[0]["google_id"]);
  ...
  {      
    ...
    $data['ecommerce_config'] = $ecommerce_config;
    $data["social_analytics_codes"] = $this->store_locale_analytics($webhook_data_final["pixel_id"],$webhook_data_final["google_id"]);
    $this->load->view('ecommerce/bare-theme', $data);      
  ...
  public function order($id=0) // if $id passed means not ajax, it's loading view
  {
    ...
    if($subscriber_id!="")
    {
      ...
      $payment_status_message=$payment_status='';
        ...
        {
          $invoice_link = base_url("ecommerce/my_orders/".$webhook_data_final['store_id']."?subscriber_id=".$subscriber_id);
          $message = "<i class='fas fa-check-circle'></i> ".$this->lang->line('Your payment has been received successfully and order will be processed soon. You can see your order status from this page')." : <br><a href='".$invoice_link."'>".$invoice_link."</a>";
          $payment_status='1';
          $payment_status_message=$message;
        }
        ...
        {
          $invoice_link = base_url("ecommerce/my_orders/".$webhook_data_final['store_id']."?subscriber_id=".$subscriber_id);
          $message = "<i class='fas fa-check-circle'></i> ".$this->lang->line('Your order has been placed successfully and is now being reviewed. You can see your order status from this page')." : <br><a href='".$invoice_link."'>".$invoice_link."</a>";
          $payment_status='1';
          $payment_status_message=$message;
        }
	  	...
	  	if($payment_status=='1')
		  $pay_message = "<div class='alert alert-success text-center'>".$payment_status_message."</div>";
		  else if($payment_status=='0')
		  $pay_message = "<div class='alert alert-danger text-center'>".$payment_status_message."</div>"; 	  
    }
        ...
        if($cart_data[0]["store_unique_id"]!="") // store availabe and online
        {
          ...
          foreach($cart_item_data as $key => $value)
          {
            ...
            if(array_key_exists($product_id, $product_list_assoc))
            {
              ...
              $attribute_info  = json_decode($cart_item_data_new[$product_id]['attribute_info'],true);
  ...
  public function proceed_checkout()
  {
    ...
    { 
	    ...
	    $this->stripe_class_ecommerce->action_url=base_url("ecommerce/stripe_action/".$user_id.'/'.$cart_id.'/'.$subscriber_id.'/'.$payment_amount.'/'.$currency.'/'.urlencode($store_name));
    ...
    { 
      ...
      $this->razorpay_class_ecommerce->action_url=base_url("ecommerce/razorpay_action/".$user_id.'/'.$cart_id.'/'.$subscriber_id); 
      $this->razorpay_class_ecommerce->currency=$currency;
      $this->razorpay_class_ecommerce->img_url=$store_favicon;
      $this->razorpay_class_ecommerce->customer_name=$buyer_first_name." ".$buyer_last_name;
      $this->razorpay_class_ecommerce->customer_email=$buyer_email;
      $this->razorpay_class_ecommerce->button_lang=$this->lang->line("Pay with Razorpay");

      // for action function, because it's not web hook based, it's js based
      // $this->session->set_userdata('ecommerce_razorpay_payment_user_id',$user_id);
      // $this->session->set_userdata('ecommerce_razorpay_payment_cart_id',$cart_id);
      // $this->session->set_userdata('ecommerce_razorpay_payment_subscriber_id',$subscriber_id);

      $razorpay_button =  $this->razorpay_class_ecommerce->set_button();
      $razorpay_button = '<div class="col-12 col-md-3"><br>'.$razorpay_button.'</div>';
    }
    ...
    { 
      ...
      $this->paystack_class_ecommerce->action_url=base_url("ecommerce/paystack_action/".$user_id.'/'.$cart_id.'/'.$subscriber_id); 
      $this->paystack_class_ecommerce->currency=$currency;
      $this->paystack_class_ecommerce->img_url=$store_favicon;
      $this->paystack_class_ecommerce->customer_first_name=$buyer_first_name;
      $this->paystack_class_ecommerce->customer_last_name=$buyer_last_name;
      $this->paystack_class_ecommerce->customer_email=$buyer_email;
      $this->paystack_class_ecommerce->button_lang=$this->lang->line("Pay with Paystack");

      // for action function, because it's not web hook based, it's js based
      // $this->session->set_userdata('ecommerce_paystack_payment_user_id',$user_id);
      // $this->session->set_userdata('ecommerce_paystack_payment_cart_id',$cart_id);
      // $this->session->set_userdata('ecommerce_paystack_payment_subscriber_id',$subscriber_id);

      $paystack_button =  $this->paystack_class_ecommerce->set_button();
      $paystack_button = '<div class="col-12 col-md-3"><br>'.$paystack_button.'</div>';
    }
    ...
    { 
      ...
      $this->mollie_class_ecommerce->action_url=base_url("ecommerce/mollie_action/".$user_id.'/'.$cart_id.'/'.$subscriber_id); 
      $this->mollie_class_ecommerce->currency=$currency;
      $this->mollie_class_ecommerce->img_url=$store_favicon;
      $this->mollie_class_ecommerce->customer_name=$buyer_first_name." ".$buyer_last_name;
      $this->mollie_class_ecommerce->customer_email=$buyer_email;
      $this->mollie_class_ecommerce->ec_order_id=$cart_id;
      $this->mollie_class_ecommerce->button_lang=$this->lang->line("Pay with Mollie");

      // for action function, because it's not web hook based, it's js based
      // $this->session->set_userdata('ecommerce_mollie_payment_user_id',$user_id);
      // $this->session->set_userdata('ecommerce_mollie_payment_cart_id',$cart_id);
      // $this->session->set_userdata('ecommerce_mollie_payment_subscriber_id',$subscriber_id);

      $mollie_button =  $this->mollie_class_ecommerce->set_button_ecommerce();
      $mollie_button = '<div class="col-12 col-md-3"><br>'.$mollie_button.'</div>';
    }

		if($manual_enabled=='1')
		{
			$manual_button = '<div class="col-12 col-md-3 text-center"><br><button id="manual-payment-button" class="btn btn-info btn-lg">'.$this->lang->line("Manual Payment").'</button><br><a href="#" id="show_manual_payment_instructions" class="pointer text-danger font-weight-bold" data-toggle="modal" data-target="#manual-payment-ins-modal"><i class="fas fa-exclamation-circle"></i> '.$this->lang->line("How to pay?").'</a></div>';
		} 

		if($cod_enabled=='1')
		{
			$cod_button = '<div class="col-12 col-md-3"><br><button id="cod-payment-button" class="btn btn-info btn-lg">'.$this->lang->line("Cash on Delivery").'</button></div>';
		}          

	  $html ='<div class="section-title m-0">'.$this->lang->line("Payment Options").'</div><div class="row">'.$paypal_button.$stripe_button.$razorpay_button.$paystack_button.$mollie_button.$cod_button.$manual_button.'</div>';
  ...
  public function stripe_action($user_id='',$cart_id='',$subscriber_id='',$payment_amount='',$currency='',$store_name='')
  {	
  		$this->load->library('stripe_class_ecommerce');
  ...
  public function razorpay_action($user_id='',$cart_id='',$subscriber_id='',$raz_order_id_session="")
  { 
      $this->load->library('razorpay_class_ecommerce');    
  ...
  public function paystack_action($user_id='',$cart_id='',$subscriber_id='',$reference="")
  { 
      $this->load->library('paystack_class_ecommerce'); 
  ...
  public function mollie_action($user_id='',$cart_id='',$subscriber_id='')
  { 
      $this->load->library('mollie_class_ecommerce'); 
  ...
  private function confirmation_message_sender($cart_id=0,$subscriber_id="")
  {
    ...
    $replace_variables = array(
      ...
      "last_name"=>$buyer_first_name,
      "first_name"=>$buyer_last_name,
      "email"=>$buyer_email,
      "mobile"=>$buyer_mobile
    );
  ...
  {
    $subscriber_id = $this->input->get("subscriber_id",true);
    $store_data = $this->basic->get_data("ecommerce_store",array("where"=>array("id"=>$store_id)),"store_name,store_unique_id,store_logo,terms_use_link,refund_policy_link,store_locale,pixel_id,google_id");
  ...
  public function change_payment_status()
  {
    ...
    {      
      ...
      if($messenger_text!="")
      {
       ...
       $api_response['messenger'] = $this->send_messenger_reminder(json_encode($messenger_confirmation_template),$page_access_token);
      }
      //  Messenger Sending block  
...
{{store_name}}')
  ...
  {    
    $store_data=$this->basic->get_data("ecommerce_store",array("where"=>array("store_unique_id"=>$store_unique_id)),'store_favicon,store_logo,store_name,store_name,id,terms_use_link,refund_policy_link,store_unique_id,store_locale,pixel_id,google_id');
